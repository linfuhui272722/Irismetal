...
public final class MetalVertexDescriptor {
    ...
    private static int getMetalFormatFromGlslType(String glslType, String name) {
        // 特殊处理：iris_UV2 和 iris_UV1 总是使用 Short2 格式
        // 因为 Minecraft 26.2 的顶点格式中，UV2 (lightmap) 是 RG16_SINT（有符号）
        // shader 中声明为 ivec2，对应 Metal 的 short2（有符号短整型）
        if (name.equals("iris_UV2") || name.equals("iris_UV1")) {
            return FORMAT_SHORT2;  // 使用有符号短整型 (ivec2 -> short2)
        }
